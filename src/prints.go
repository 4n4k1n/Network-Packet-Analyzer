...
func printStats(stats_data Stats_data) {
	...
	fmt.Printf("Average rate            : %f\n", stats_data.average_rate)
	fmt.Printf("Unique source IPs       : %d\n", stats_data.unique_src_ids)
	fmt.Printf("Unique destination IPs  : %d\n", stats_data.unique_dst_ids)
	fmt.Printf("Protocol breakdown      :\n")
	fmt.Printf("TCP : %d packets (%.1f)", stats_data.tcp_packets, percent.PercentOf(stats_data.tcp_packets, stats_data.total_packets))
	fmt.Printf("UDP : %d packets (%.1f)", stats_data.udp_packets, percent.PercentOf(stats_data.udp_packets, stats_data.total_packets))
